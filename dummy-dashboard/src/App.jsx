...
function App() {
    ...
    const initializeMap = () => {  // Separate function for map initialization
      ...
      map.on("click", async (e) => {
        ...
        try {
          ...
          const digitizedRes = await fetch(`https://13.51.168.78:8000/api/digitized-parcel/?${coordinateString}`);
          ...
          // Try MergedSociety
          const mergedRes = await fetch(`https://13.51.168.78:8000/api/land-parcel/?${coordinateString}`);
          ...
          const societyRes = await fetch(`https://13.51.168.78:8000/api/society-parcel/?${coordinateString}`);
  ...
  useEffect(() => {
    ...
    // axios.get('https://13.51.168.78:8000/api/joined-mauza-districts/')
    axios.get('https://13.51.168.78:8000/api/societies/')
      .then((response) => {
  ...
  const fetchFilteredData = () => {
    ...
    axios
      .get('https://13.51.168.78:8000/api/societies/', { params })
      .then((response) => {
  ...
  const fetchNewFilteredData = () => {
    ...
    axios
      .get('https://13.51.168.78:8000/api/joined-mauza-districts/', { params })
      .then((response) => {
        const geojson = {
          type: 'FeatureCollection',
          features: response.data.map((feature) => {
  ...
  const handleDistrictChange = (district) => {
    ...
    axios
      // .get('https://13.51.168.78:8000/api/joined-mauza-districts/', {
      .get('https://13.51.168.78:8000/api/societies/', {
        params: { district },
      })
      .then((response) => {
        const uniqueTehsils = [
  ...
  const toggleLayerVisible = async (town, isVisible) => {
    ...
    if (isVisible) {
      const response = await fetch(`https://13.51.168.78:8000/api/geojson/?town_name=${encodeURIComponent(town)}`);
  ...
  const zoomToLayer = async (town) => {
    const map = mapRef.current;
    const response = await fetch(`https://13.51.168.78:8000/api/bbox/?town_name=${encodeURIComponent(town)}`);
  ...
  const fetchMBlockData = async () => {
    try {
      const response = await fetch('https://13.51.168.78:8000/api/m-block/');
      const geojson = await response.json();
  ...
  const zoomToMBlock = async () => {
    const map = mapRef.current;
    const response = await fetch('https://13.51.168.78:8000/api/m-block-bbox/');
    const { bbox } = await response.json(); // Expecting { bbox: [minLng, minLat, maxLng, maxLat] }
