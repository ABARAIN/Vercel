...
const SpatialQuery = ({ map, geojsonData, setGeojsonData, landuseFilter, setFullGeojsonBackup,setClickedPlotDetails }) => {
  ...
  useEffect(() => {
    axios.get('https://13.51.168.78:8000/api/all-soc-dropdowns/?level=district')
  ...
  useEffect(() => {
    ...
    if (selectedDistrict) {
      axios.get(`https://13.51.168.78:8000/api/all-soc-dropdowns/?level=tehsil&district=${selectedDistrict}`)
  ...
  useEffect(() => {
    ...
    if (selectedDistrict && selectedTehsil) {
      axios.get(`https://13.51.168.78:8000/api/all-soc-dropdowns/?level=town_name&district=${selectedDistrict}&tehsil=${selectedTehsil}`)
  ...
  useEffect(() => {
    ...
    if (selectedDistrict && selectedTehsil && selectedTown) {
      axios.get(`https://13.51.168.78:8000/api/all-soc-dropdowns/?level=block&district=${selectedDistrict}&tehsil=${selectedTehsil}&town_name=${selectedTown}`)
  ...
  useEffect(() => {
    ...
    if (selectedDistrict && selectedTehsil && selectedTown && selectedBlock) {
      axios.get(`https://13.51.168.78:8000/api/all-soc-dropdowns/?level=plot_no&district=${selectedDistrict}&tehsil=${selectedTehsil}&town_name=${selectedTown}&block=${selectedBlock}`)
        .then(res => {
  ...
  const handleShow = async () => {
    ...
    try {
      const res = await axios.get('https://13.51.168.78:8000/api/all-soc-geojson/', { params });
      ...
      //   try {
      //     const popupRes = await fetch(`https://13.51.168.78:8000/api/society-parcel/?${coordinateString}`);
